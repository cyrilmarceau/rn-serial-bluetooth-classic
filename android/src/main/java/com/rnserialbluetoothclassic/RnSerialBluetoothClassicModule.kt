...
class RnSerialBluetoothClassicModule(reactContext: ReactApplicationContext) :
    ...
    override fun connect(address: String, options: ReadableMap, promise: Promise) {
        Log.d("Connect", "Options $options")

        deviceManager.connect(address = address, options = options, promise = promise)
    }

    override fun disconnect(promise: Promise) {
        deviceManager.disconnect(promise)
    }

    override fun isConnected(promise: Promise) {
        deviceManager.isConnected(promise)
    }

    override fun write(data: String, promise: Promise) {
        deviceManager.write(data, promise)
    }
